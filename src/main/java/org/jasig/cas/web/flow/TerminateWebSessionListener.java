...
/**
 * Listener to expire site session as soon as the webflow is ended. The goal is to decrease memory
 * consumption by deleting as soon as
 * possible the site sessions created mainly for login process.
 *
 * @author Jerome Leleu
...
public final class TerminateWebSessionListener extends FlowExecutionListenerAdapter {
    ...
    public void sessionEnded(final RequestContext context, final FlowSession session, final String outcome,
        ...
        if ( session.isRoot() ) {
            ...
            if (webSession != null) {
                LOGGER.debug("Terminate site session {} in {} seconds", webSession.getId(), this.timeToDieInSeconds);
                // set the site session to die in timeToDieInSeconds
